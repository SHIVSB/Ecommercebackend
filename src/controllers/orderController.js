...
const {httpCodes} = require("../constants/backendConfig");
                module.exports = {
                    ...
                    getOrderDetails: function (req, res) {
                        ...
                        if (data.userId) {
                            OrderDetails.getOrderDetails(data, function (err, result) {
                                if (err) {
                    ...
                    editOrder: function (req, res) {
                        ...
                        if (data.orderId && data.userId) {
                            if(data.payment) {
                                OrderDetails.editOrder(data, function(err){
                                    if (err) {
                                ...
                                Product.getProductDetails(data, function(err, result){
                                    ...
                                    OrderDetails.getOrderDetails(data, function(err1, result1){
                                        if (err1) {
                                            responseData.msg = "Error in updating order details";
                                            return res.status(httpCodes.internalServerError).send(responseData);
                                        }
                                        if(data.remove) {
                                            OrderItem.deleteOrderItem(data, function(err2){
                                                ...
                                                OrderDetails.editOrder(data, function(err3){
                                                    if (err3) {
                                                        responseData.msg = "Error in updating order details";
                                                        return res.status(httpCodes.internalServerError).send(responseData);
                                                    }
